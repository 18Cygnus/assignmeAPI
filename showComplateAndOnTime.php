...
if ($con) {
    ...
    if (mysqli_num_rows($resultUserId) > 0) {
        ...
        // Query untuk mendapatkan daftar tugas yang sudah dikumpulkan dan tidak terlambat dari kelas yang di-join oleh pengguna berdasarkan UserId 
    
        $sql = "SELECT tasks.TaskId, tasks.TaskName, tasks.TaskDesc, tasks.DueDate, task_submits.SubmitDate
        FROM tasks
        JOIN task_submits ON tasks.TaskId = task_submits.TaskId
        JOIN user_classes ON user_classes.ClassId = tasks.ClassId
        WHERE user_classes.UserId = ? AND task_submits.SubmitId IS NOT NULL AND task_submits.SubmitDate <= tasks.DueDate";       
